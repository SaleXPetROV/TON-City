...
async def verify_wallet(request: WalletVerifyRequest):
    ...
    try:
        ...
        if not user_doc:
            new_user = User(wallet_address=wallet_address, language=request.language)
            ...
            await db.users.update_one(
                ...
                {"$set": {"last_login": datetime.now(timezone.utc).isoformat(), "language": request.language}}
            )
...
async def purchase_plot(request: PurchasePlotRequest, current_user: User = Depends(get_current_user)):
    ...
    return {
        ...
        "amount_ton": plot["price"],
        "zone": zone,
        "message": f"Purchase plot ({x}, {y}) for {plot['price']} TON"
    }
...
async def build_business(request: BuildBusinessRequest, current_user: User = Depends(get_current_user)):
    ...
    # Check global limits (like exchanges)
    if "max_total" in bt:
    ...
    build_dict = build_order.model_dump()
    build_dict['created_at'] = build_dict['created_at'].isoformat()
    await db.build_orders.insert_one(build_dict.copy())
    ...
    return {
        ...
        "business_type": request.business_type,
        "amount_ton": total_cost,
        "materials_required": bt["materials_required"],
        "min_builders": bt["min_builders"],
        "base_build_time_hours": bt["build_time_hours"],
        "message": f"Build {bt['name']['en']} for {total_cost} TON"
    }
