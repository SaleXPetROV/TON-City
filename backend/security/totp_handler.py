...
def create_totp_routes(db):
    ...
    async def confirm_disable_2fa(request: TOTPDisableConfirmRequest, request_id: str, current_user: dict):
        ...
        await db.users.update_one(
            {"id": user["id"]},
            {
                "$set": {
                    ...
                    "withdraw_lock_until": (datetime.now(timezone.utc) + timedelta(hours=48)).isoformat()
                },
                "$unset": {
                    "two_factor_secret": "",
                    "backup_codes": "",
        ...
        await security_service._log_security_event(
            user["id"], "2fa_disabled", {"withdraw_locked_hours": 48}
        )
        
        return {
            "status": "disabled",
            "message": "2FA отключена. Вывод средств заблокирован на 48 часов.",
            "withdraw_lock_hours": 48
        }
