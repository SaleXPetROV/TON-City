...
# Import new business system
from business_config import (
    BUSINESSES, TIER_TAXES, PATRON_BONUSES, RESOURCE_WEIGHTS,
    WAREHOUSE_CONFIG, PATRON_TAX_RATE, INSTANT_WITHDRAWAL_FEE,
    calculate_upgrade_cost, calculate_production, calculate_income,
    calculate_repair_cost, get_daily_wear, get_storage_capacity, get_patron_bonus,
    check_resource_requirements, LEVEL_CONFIG as BIZ_LEVEL_CONFIG
)
...
async def upgrade_business(business_id: str, current_user: User = Depends(get_current_user)):
    ...
    if business.get("owner") != current_user.wallet_address:
    ...
    logger.info(f"Business {business_id} upgraded to level {upgrade_data['level']} by {current_user.wallet_address}")
...
async def repair_business(business_id: str, current_user: User = Depends(get_current_user)):
    ...
    logger.info(f"Business {business_id} repaired by {current_user.wallet_address} for {cost} TON")
...
async def instant_withdrawal(
    ...
    user = await db.users.find_one({"wallet_address": current_user.wallet_address}, {"_id": 0})
    if not user or user.get("balance_ton", 0) < amount:
        raise HTTPException(status_code=400, detail="Недостаточно средств")
...
async def create_land_listing(data: LandListingRequest, current_user: User = Depends(get_current_user)):
    ...
    if city:
        name = city.get("name")
        if isinstance(name, dict):
            city_name = name.get("ru") or name.get("en") or "Unknown"
