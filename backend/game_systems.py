...
class BusinessEconomics:
    ...
    def calculate_effective_production(business: dict, patron_bonus: float = 1.0) -> dict:
        ...
        return BusinessEconomics.calculate_tick_output(business, patron_bonus)
    
    @staticmethod
    def calculate_tick_output(business: dict, patron_bonus: float = 1.0) -> dict:
        """
        ...
        # Production = base * (durability/100) * patron_bonus
        raw_production = get_production(business_type, level)
        durability_mult = durability / 100.0
        effective_production = raw_production * durability_mult * patron_bonus
...
class EconomicTickEngine:
    ...
    def process_tick_for_business(business: dict, available_resources: dict, 
        ...
        # === STEP 1: PRODUCTION ===
        tick_output = BusinessEconomics.calculate_tick_output(business, patron_bonus)
...
class IncomeCollector:
    ...
    def calculate_pending_income(business: dict, hours_passed: float = None, 
                                  patron_bonus: float = 1.0, market_prices: dict = None) -> dict:
