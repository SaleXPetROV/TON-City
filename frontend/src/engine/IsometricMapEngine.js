...
export class IsometricMapEngine {
  ...
  async setupAllTiles(state) {
    ...
    for (const cell of sortedCells) {
      ...
      // Collect cells with owner avatars (show on ALL owned tiles without buildings)
      if (cell.owner && !cell.building) {
  ...
  createAvatar(cell, state) {
    ...
    // Размер аватарки (в пикселях)
    const avatarSize = 24;
    
    if (!texture) {
      // Create fallback with initials - ромбовидная форма
      const initial = (cell.ownerUsername || 'U')[0].toUpperCase();
      
      // Ромбовидный фон (изометрический алмаз)
      const diamond = new Graphics();
      const halfW = avatarSize / 2;
      const halfH = avatarSize / 2 * 0.5; // Изометрическая пропорция
      diamond.poly([
      ...
      // Create avatar sprite with texture
      ...
      // Создаём маску ромбовидной формы
    ...
    // Position at center of tile - чуть выше для лучшей видимости
    avatarContainer.x = snapToGrid(pos.x + TILE_WIDTH / 2);
    avatarContainer.y = snapToGrid(pos.y + TILE_HEIGHT / 2 - 4); // Приподнимаем на 4px
    avatarContainer.zIndex = (cell.q + cell.r) + 0.5;
  ...
  updateCell(cell, state) {
    ...
    if (cell.building) {
      ...
      // Remove avatar if building exists
    ...
    // Avatar update - show for ALL owned tiles without buildings
    const avatarSprite = this.avatarPool.get(key);
    if (cell.owner && !cell.building) {
      if (!avatarSprite) {
