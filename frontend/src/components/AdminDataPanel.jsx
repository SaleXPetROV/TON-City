...
import { Search, User, Package, Edit2, Save, AlertTriangle, TrendingUp, Shield, X } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {playerDetails && (
          <div className="p-4 rounded-2xl bg-gray-800/40 border border-gray-700/50 space-y-4 max-h-[600px] overflow-y-auto">
            ...
            {/* Device / Multi-account */}
            <div className="p-3 rounded-xl bg-gray-900/50 border border-gray-700/30">
              <div className="text-xs font-bold text-gray-400 mb-2">📱 Устройство</div>
              <div className="text-xs text-gray-500 font-mono break-all">{playerDetails.device_fingerprint || 'Не определено'}</div>
...
function PricesTab({ token, headers }) {
  ...
  const stabilizeResource = async (resource) => {
    ...
    try {
      const res = await axios.post(`${API}/admin/market/stabilize?resource=${resource}&target_price=${target}`, {}, { headers });
      toast.success(`Бот-стабилизатор запущен для ${resource}. Продажа по ${(target * 1.02).toFixed(4)} TON. Действует 40 минут.`);
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-bold text-gray-300">💰 Управление ценами ресурсов</h3>
        <Button onClick={savePrices} size="sm" className="bg-green-600 hover:bg-green-700">
          <Save className="w-3 h-3 mr-1" /> Сохранить изменения
        </Button>
      </div>

      <div className="text-xs text-gray-500 p-2 rounded-lg bg-gray-900/30 border border-gray-700/30">
        ⚠️ Минимальная цена: <span className="text-amber-400 font-bold">0.01 TON</span>. Ни один товар не может стоить дешевле.
        <br />🤖 Кнопка "Стабилизатор" запускает NPC-бота, который продаёт ресурс по цене +2% от целевой на 40 минут.
      </div>
        ...
        {Object.entries(prices).sort((a, b) => (a[1].tier || 0) - (b[1].tier || 0)).map(([resource, data]) => (
          <div key={resource} className="flex items-center gap-3 p-3 rounded-xl bg-gray-800/30 border border-gray-700/30">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-500">Tier {data.tier} • База: {data.base_price} TON</div>
            </div>
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-gray-500 w-8">TON</span>
              <Button size="sm" variant="ghost" onClick={() => stabilizeResource(resource)} className="text-amber-400 hover:bg-amber-900/20 text-xs h-8 px-2" title="Запустить стабилизатор">
