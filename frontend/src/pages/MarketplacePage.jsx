...
export default function MarketplacePage({ user, refreshBalance, updateBalance }) {
  ...
  const openBuyLandModal = (listing) => {
    ...
    if (sellerId === user?.id || sellerId === user?.wallet_address) {
      toast.error('Нельзя купить свой собственный листинг');
      return;
    }
                ...
                <div className="flex flex-wrap gap-3 mb-4">
                  ...
                  <Select value={businessTypeFilter} onValueChange={setBusinessTypeFilter}>
                    ...
                    <SelectContent className="max-h-80">
                      ...
                      {/* Tier 1 - Базовые */}
                      <SelectItem value="helios">⚡ Гелиос (Энергия)</SelectItem>
                      <SelectItem value="quartz_mine">💎 Кварцевый рудник</SelectItem>
                      <SelectItem value="node_hub">📡 Нодовый хаб</SelectItem>
                      <SelectItem value="cryo_farm">❄️ Крио-ферма</SelectItem>
                      <SelectItem value="bio_reactor">🌿 Био-реактор</SelectItem>
                      <SelectItem value="recycler">♻️ Переработчик</SelectItem>
                      <SelectItem value="repair_shop">🔧 Ремонтная мастерская</SelectItem>
                      {/* Tier 2 - Производство */}
                      <SelectItem value="chip_forge">🔧 Чип-кузница</SelectItem>
                      <SelectItem value="nft_mint">🎨 NFT-минт</SelectItem>
                      <SelectItem value="neuro_lab">🧠 Нейролаборатория</SelectItem>
                      <SelectItem value="logistics_hub">🚚 Логистический центр</SelectItem>
                      <SelectItem value="vr_arcade">🎮 VR-аркада</SelectItem>
                      <SelectItem value="data_center">🖥️ Дата-центр</SelectItem>
                      <SelectItem value="smart_factory">🏭 Смарт-фабрика</SelectItem>
                      {/* Tier 3 - Крупные */}
                      <SelectItem value="ton_casino">🎰 TON Казино</SelectItem>
                      <SelectItem value="stock_exchange">📈 Биржа TON</SelectItem>
                      <SelectItem value="research_institute">🔬 Исследовательский институт</SelectItem>
                      <SelectItem value="spaceport">🚀 Космопорт</SelectItem>
                      <SelectItem value="quantum_computer">💫 Квантовый компьютер</SelectItem>
                      <SelectItem value="ai_nexus">🤖 AI Нексус</SelectItem>
                      <SelectItem value="mega_mall">🏬 Мега-молл</SelectItem>
                    </SelectContent>
                  </Select>
                        ...
                        <Card key={listing.id} className="glass-panel border-white/10 hover:border-amber-500/50 transition-all">
                          <CardContent className="p-4">
                            ...
                            >
                              ...
                              {((listing.seller_id || listing.seller_user_id) === user?.id || (listing.seller_id || listing.seller_user_id) === user?.wallet_address) ? 'Ваш листинг' : 'Купить участок'}
                            </Button>
                          </CardContent>
                        </Card>
                      );
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {myLandListings.filter(l => l.status === 'active').length === 0 ? (
                        ...
                        // Combine myLandListings with any listings from landListings that belong to the user
                        [...myLandListings, ...landListings.filter(l => {
                          const sellerId = l.seller_id || l.seller_user_id;
                          return (sellerId === user?.id || sellerId === user?.wallet_address) && !myLandListings.find(m => m.id === l.id);
                        })]
                          .filter(l => l.status === 'active').map(listing => {
                            ...
                            <CardContent className="p-3">
                              <div className="flex items-center justify-between mb-2">
                                <div>
                                  ...
                                  <div className="text-xs text-amber-400">{(listing.price || 0).toFixed(2)} TON</div>
                                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-white/5 rounded-lg border border-white/10">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-text-muted">Продавец:</span>
                    <span className="text-white">{selectedLandListing.seller_username || 'Аноним'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-500/10 rounded-lg border border-amber-500/20">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-bold text-amber-400">{selectedLandListing.price?.toFixed(2)} TON</span>
                </div>
                <div className="flex justify-between items-center text-sm mt-2">
                  ...
                  <span className={`font-mono ${(user?.balance_ton || 0) >= selectedLandListing.price ? 'text-green-400' : 'text-red-400'}`}>
                    {(user?.balance_ton || 0).toFixed(2)} TON
                  </span>
                </div>
              </div>
