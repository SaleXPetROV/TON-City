...
export default function GamePage({ user }) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // Update the plot in renderer
      if (rendererRef.current) {
        const updatedPlot = { ...selectedPlot, owner: user.id };
    ...
    return (
      <div className="min-h-screen bg-void flex items-center justify-center font-rajdhani">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-cyber-cyan animate-pulse">Загрузка острова...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-cyber-cyan uppercase tracking-widest">
                  {plots.length} {t('plots') || 'plots'} • Гексагональная сетка
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-1 text-xs">
              ...
              <div className="border-t border-white/10 mt-2 pt-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white">С бизнесом</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {selectedPlot && (
            <div className="space-y-4">
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                ...
                <div className="flex justify-between items-center mb-2">
                  <span className="text-text-muted text-sm">Зона</span>
                  <span className="text-cyber-cyan">
                    {selectedPlot.zone === 'core' ? 'Ядро' : 
                     selectedPlot.zone === 'inner' ? 'Внутреннее' :
                     selectedPlot.zone === 'middle' ? 'Среднее' : 'Внешнее'}
                  </span>
                </div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-text-muted text-sm">Цена</span>
                  <span className="text-signal-amber font-mono font-bold">{selectedPlot.price?.toFixed(2)} TON</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-text-muted text-sm">Ваш баланс</span>
        ...
        <DialogContent className="bg-panel border-grid-border max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <p className="text-text-muted text-sm">
              Выберите тип бизнеса для постройки. Бизнесы разделены по уровням (тирам).
            </p>
            ...
            <div>
              <h4 className="text-xs text-green-400 font-bold uppercase mb-2 flex items-center gap-2">
                ...
                Tier 1 — Базовое производство (15% налог)
              </h4>
            ...
            <div>
              <h4 className="text-xs text-purple-400 font-bold uppercase mb-2 flex items-center gap-2">
                ...
                Tier 3 — Финансы/Развлечения (30% налог) — Патроны
              </h4>
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
