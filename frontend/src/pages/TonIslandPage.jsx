...
export default function TonIslandPage({ user, refreshBalance, updateBalance }) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (updateBalance) updateBalance(newBalance);
    ...
    <div className="flex h-screen bg-void overflow-hidden">
      ...
      <div className="flex-1 flex flex-col lg:ml-16 overflow-hidden">
        ...
        <div className="flex-1 relative overflow-hidden">
          {/* Recommendations Widget - Right side, triggered by button */}
          <AnimatePresence>
            {showRecommendations && recommendations.length > 0 && (
              <motion.div
                data-testid="recommendations-widget"
                initial={{ x: 300, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                exit={{ x: 300, opacity: 0 }}
                transition={{ type: 'spring', damping: 20 }}
                className="absolute top-2 right-16 z-20 w-72 max-h-[50vh] overflow-y-auto bg-gray-950/95 backdrop-blur-xl rounded-xl border border-cyan-500/30 shadow-lg shadow-cyan-500/10"
              >
                <div className="p-3 border-b border-gray-800 flex items-center justify-between">
                  <span className="text-xs font-bold text-cyan-400 uppercase tracking-wider">Рекомендации</span>
                  <button onClick={() => setShowRecommendations(false)} className="text-gray-500 hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                  ...
                  {recommendations.map((r, i) => (
                    ...
                    >
                      ...
                      <div className="text-right shrink-0">
                        <div className="text-[10px] text-green-400 font-mono">{r.estimated_daily_income} TON/д</div>
                        <div className="text-[10px] text-gray-500">{r.cost_ton} TON</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Button
              ...
              className={`w-12 h-12 transition-all ${
                showRecommendations
                  ...
                  : 'bg-black/60 border-white/20 text-white/60 hover:bg-white/10'
              }`}
              title="Рекомендации"
            >
          ...
          {/* Selected Cell Info - Right side */}
          {selectedCell && !showPurchaseModal && !showBuildModal && !showBusinessModal && (
            <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-sm rounded-lg p-4 text-sm z-20 min-w-[220px]">
              ...
              <div className="space-y-1 text-white/80">
                ...
                {selectedCell.owner ? (
                  <div className="mt-2 pt-2 border-t border-white/20">
                    <div className="text-text-muted text-xs mb-1">Владелец:</div>
                    <div className="flex items-center gap-2">
                      {selectedCell.ownerAvatar && (
                        <img src={selectedCell.ownerAvatar} alt="" className="w-6 h-6 rounded" />
                      )}
                      <span className="text-green-400 font-medium">
                        {selectedCell.ownerUsername || 'Игрок'}
                      </span>
                    </div>
                  </div>
                ) : (
                  <div className="mt-2 pt-2 border-t border-white/20">
                    <div className="text-amber-400 text-xs">Свободен для покупки</div>
                  </div>
                )}
                ...
                {selectedCell.building && (
                  <div className="mt-2 pt-2 border-t border-white/20">
                    <div className="text-text-muted text-xs mb-1">Бизнес:</div>
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-purple-400">
                          Уровень {selectedCell.building.level} • Tier {selectedCell.building.tier || 1}
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-white/5 p-3 rounded-lg">
                  ...
                  <div className="text-white font-bold">{ZONE_NAMES[selectedCell.zone] || selectedCell.zone}</div>
                </div>
              </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-text-muted">
                        {config.base_cost_ton || config.cost || '?'} TON • Производит: {RESOURCE_NAMES[config.produces] || config.produces || 'TON'}
                      </div>
                    </div>
