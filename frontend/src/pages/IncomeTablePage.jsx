...
export default function IncomeTablePage({ user }) {
  ...
  const calculateIncome = () => {
    ...
    const baseProduction = biz.levels?.[level]?.production || biz.production_rate || 100;
          ...
          <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 pb-24">
            ...
            <div className="sticky top-0 z-10 bg-gray-950/90 backdrop-blur border-b border-gray-800 px-4 py-3">
              <div className="max-w-4xl mx-auto flex items-center gap-3">
                ...
                <h1 className="text-lg font-bold text-white">Калькулятор доходов</h1>
              </div>
            </div>
              ...
              <Card className="bg-gray-900/50 border-gray-800">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    ...
                    Рассчитать доход бизнеса
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-6">
                  {/* Business Selector */}
                  <div className="space-y-2">
                    <Label className="text-gray-400">Выберите бизнес</Label>
                  ...
                  {/* Level Selector */}
                  <div className="space-y-2">
                    <Label className="text-gray-400">Уровень бизнеса: {level}</Label>
                    ...
                    <div className="text-xs text-gray-500">
                      Множитель производства: x{LEVEL_MULTIPLIERS[level]}
                    </div>
                  </div>

                  {/* Durability Slider */}
                  <div className="space-y-2">
                    <Label className="text-gray-400">Прочность: {durability}%</Label>
                    <Slider
                      value={[durability]}
                    ...
                    <div className="flex justify-between text-xs text-gray-500">
                      ...
                      <span className={durability < 30 ? 'text-red-400' : durability < 70 ? 'text-yellow-400' : 'text-green-400'}>
                        {durability < 30 ? 'Критически низкая' : durability < 70 ? 'Средняя' : 'Хорошая'}
                      </span>
                      <span>100%</span>
                    </div>
                  </div>
                  ...
                  {/* Price Per Unit */}
                  <div className="space-y-2">
                    <Label className="text-gray-400">Цена за единицу товара (TON)</Label>
                    <Input
                      type="number"
                      step="0.001"
                      min="0.001"
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="p-4 rounded-xl bg-gray-800/50 border border-gray-700">
                          <div className="text-xs text-gray-400 uppercase mb-1">Производство в час</div>
                          <div className="text-xl font-bold text-cyan-400">
                            {result.productionPerHour} {getResource(result.produces)?.icon}
                          </div>
                        </div>
                        <div className="p-4 rounded-xl bg-gray-800/50 border border-gray-700">
                          <div className="text-xs text-gray-400 uppercase mb-1">Производство в сутки</div>
                          <div className="text-xl font-bold text-cyan-400">
                            {result.productionPerDay} {getResource(result.produces)?.icon}
                          </div>
                        </div>
                        <div className="p-4 rounded-xl bg-gray-800/50 border border-gray-700">
                          <div className="text-xs text-gray-400 uppercase mb-1">Производство в месяц</div>
                          <div className="text-xl font-bold text-cyan-400">
