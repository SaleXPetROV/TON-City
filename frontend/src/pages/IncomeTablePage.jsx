...
import { Calculator, TrendingUp, Building2, ArrowLeft, GraduationCap } from 'lucide-react';
    ...
    <div className="min-h-screen bg-void">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <motion.div
          ...
          className="glass-panel rounded-xl p-6 text-center mt-6"
        >
          <h3 className="font-unbounded text-text-main mb-3">
            {lang === 'ru' ? 'Хотите узнать больше?' : lang === 'zh' ? '想了解更多？' : 'Want to learn more?'}
          </h3>
          <p className="text-text-muted mb-4">
            {lang === 'ru' 
              ? 'Пройдите интерактивное обучение чтобы понять как все элементы игры связаны между собой'
              : lang === 'zh' 
              ? '通过互动教程了解游戏中各元素之间的关系'
              : 'Take an interactive tutorial to understand how all game elements are connected'
            }
          </p>
          <Button
            onClick={() => setShowTutorial(true)}
            className="btn-cyber px-6 py-3"
            data-testid="income-tutorial-btn"
          >
            <GraduationCap className="w-5 h-5 mr-2" />
            {lang === 'ru' ? 'Пройти обучение' : lang === 'zh' ? '开始教程' : 'Take Tutorial'}
          </Button>
        </motion.div>
      </main>
